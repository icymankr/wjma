...
//  dmMain:= TdmMain.Create(AOwner);
  fmProgPriv := TfmProgPriv.Create(AOwner);
  fmProgPriv.ModuleID:= ModuleID;
  fmProgPriv.BorderStyle:= bsNone;
  fmProgPriv.Align:= alClient;
  fmProgPriv.Parent:= Parent;
  If Visible = True then fmProgPriv.Show;
End;
